(function($) {
    jQuery.fn.drop_uploader = function(options) {
        options = $.extend({
            ...
            file_icon: '<i class="bi bi-cloud-arrow-up"></i>',
        ...
        this.each(function(i, val) {
            ...
            function preview_file(file, i) {
                ...
                reader.onloadend = function() {
                    ...
                    // Add delete (cross) button
                    $('#' + uploader_id + ' #selected_file_' + i + ' div.thumbnail').append('<button class="delete-btn" data-index="' + i + '">X</button>');
            
                    // Add event listener to remove the image on click of delete button
                    $('#' + uploader_id + ' #selected_file_' + i + ' .delete-btn').on('click', function() {
                        // Remove the selected image
                        $('#' + uploader_id + ' #selected_file_' + i).remove();
                    });
                };
            }
            ...
            function file_upload_ajax(file, i) {
                ...
                var csrfTokenValue = $('#csrf_token_hash').val();
            
                var formData = new FormData();
                formData.append(input_name.replace('[]', ''), file);
                formData.append(csrfTokenName, csrfTokenValue);

                // ajaxPostRequest(submit_url, formData, function(data) {
                ...
                $.ajax({
                    ...
                    //                 var progress_el = $('#' + uploader_id + ' #selected_file_' + i + ' .du_progress');
                    //                 draw_round_progress(progress_el[0], percent / 100, layout);
                    //             }
                    //         }, false);
                    //     }
                    //     return xhr;
