...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		$data = [
			...
			'created_on' => time(),
			'is_active' => ($manual_activation === FALSE ? 'Y' : 'N')
		];
	...
	public function login($identity, $password, $remember = FALSE)
	{
		...
		if ($query->num_rows() === 1) {
			...
			//MODIFY BY Suhrid Sarkar || suhrid.developer@gmail.com
			//For Temp Password Check 
			//ON JUNE 2, 2022
			else{
				$check_temp = $this->db->select('*')->from('temp_password')->where('user_id', $user->id)->get();
				// echo $check_temp->num_rows(); die();
				if($check_temp->num_rows() === 1){
					// echo $user->is_active; die();
					if($user->is_active == 'N'){
						$this->trigger_events('post_login_unsuccessful');
						$this->set_error('login_unsuccessful_not_active');
					...
					// if ($this->verify_password($password, $check_temp->row()->temp_password, $identity)){
					if ($password === $check_temp->row()->temp_password){

						$valid_time = date('Y-m-d H:i', strtotime($check_temp->row()->valid_till));
	...
	public function user_gp_privilegs($gp_id)
	{
		...
		$this->db->order_by('permissions.perm_sort', 'asc');
		$query = $this->db->get();
		return $query->result();
	}
	
	public function gp_perm_submodules($perm_id = '', $parent = 0, $gp_id='')
	{
		$this->db->select('*');
		$this->db->from('permission_submodules');
		$this->db->join('group_sub_perm', 'group_sub_perm.perm_sub_id = permission_submodules.perm_sub_id AND group_sub_perm.group_id = '.$gp_id); //CR ID-6-220127
		$this->db->where('perm_id', $perm_id);
		$this->db->where('parent', $parent);
		$this->db->group_by('permission_submodules.perm_sub_id');
		$this->db->order_by('permission_submodules.perm_sub_sort', 'asc');
		$query = $this->db->get();
		return $query->result();
	}


	//added by monish on 25-10-21
	public function check_perm($group_id = '', $perm_name = '')
	{
		$this->db->select('permissions.perm_id');
		$this->db->from('group_perm');
		$this->db->join('permissions', 'group_perm.perm_id = permissions.perm_id');
	...
	//added by monish on 25-10-21
	public function check_sub_perm($group_id = '', $perm_id = '', $perm_sub_name = '')
	{
		$this->db->select('permission_submodules.perm_sub_id');
		$this->db->from('group_sub_perm');
		$this->db->join('permission_submodules', 'group_sub_perm.perm_sub_id = permission_submodules.perm_sub_id');
		$this->db->where('permission_submodules.perm_sub_name', $perm_sub_name);
		$this->db->where('permission_submodules.perm_id', $perm_id);
		$this->db->where('group_id', $group_id);
		$query = $this->db->get();
		$result = $query->row();

		if (!empty($result)) {
			return $result->perm_sub_id;
		} else {
			return FALSE;
		}
	}
	...
	//added by monish on 25-10-21
	public function check_comp_perm($group_id = '', $sub_perm_id = '', $perm_comp_name = '')
	{
		$this->db->select('permission_components.perm_comp_id');
		$this->db->from('group_comp_perm');
		$this->db->join('permission_components', 'group_comp_perm.perm_comp_id = permission_components.perm_comp_id');
		$this->db->where('permission_components.perm_comp_name', $perm_comp_name);
		$this->db->where('permission_components.perm_sub_id', $sub_perm_id);
		$this->db->where('group_comp_perm.group_id', $group_id);
		$query = $this->db->get();
		$result = $query->row();
