...
/**
 * Auther : Suhrid Sarkar || suhrid.developer@gmail.com
 * IDE    : VS Code
 * Date   : 13/02/2023
 */
...
class Common_model extends CI_Model
{
    ...
    /* 
    Get For Where In Condition
    */
    public function getWhereIn($where_in_array, $whereinField, $where_array = '', $tablename, $groupBy = '', $joins = [], $specific = '', $row = '', $or_group = '')
    {
        try {
            //code...
            // If Specific Columns are require
    ...
    /* 
    For Multiple Where in Clause
    */
    public function getMultiWhereIn($where_in_array, $where_array = '', $tablename, $groupBy = '', $joins = [], $specific = '', $order = '')
    {
        try {
            // If Specific Columns are require
            if (!empty($specific)):
            ...
            if (!empty($where_in_array)) {
                for ($wc = 0; $wc < count($where_in_array); $wc++) {
                    $this->db->where_in($where_in_array[$wc]['field'], $where_in_array[$wc]['valueArray']);
                }
            }
    ...
    */
    ...
    //Added by Suhrid Sarkar || suhrid.developer@gmail.com on April 13, 2023
	...
	//Added by Suhrid Sarkar || suhrid.developer@gmail.com on June 09, 2023
	//Modify by Suhrid Sarkar || suhrid.developer@gmail.com on June 11, 2023
    //Modify by Suhrid Sarkar || suhrid.developer@gmail.com on July 11, 2023
    // Fetch all fields for the given table
    public function get_fields($table_name) {
        return $this->db->list_fields($table_name);
    }
    // Fetch relational dropdown options for categories
    public function relational_dropdown($table, $id_field = 'id', $name_fields = '', $where = [], $separator = ' - ') {
        ...
        $name_fields_array = explode(',', $name_fields);
        $select_fields = array_merge([$id_field], $name_fields_array);
    
        // Prepare the query
        $this->db->select($select_fields);
        if (!empty($where)) {
            $this->db->where($where); // Apply any conditions passed in the $where array
        }
        $query = $this->db->get($table);
    
        // Combine the selected fields with the separator
        $result = [];
        foreach ($query->result_array() as $row) {
            $key = $row[$id_field];
            $names = [];
            foreach ($name_fields_array as $field) {
                $names[] = $row[trim($field)]; // Trim to avoid any leading/trailing spaces
            }
            $result[$key] = implode($separator, $names);
            // pr($result);
        }
	...
	function get_holiday_list(){
		$this->db->select( 'GROUP_CONCAT(date SEPARATOR ",") as dates');
		//$this->db->select( 'GROUP_CONCAT(CONCAT("\'", h_date, "\'" )) as dates');
		$result	= $this->db->get('holiday_master');
		$result	= $result->row(); 		
		return $result;
	}
