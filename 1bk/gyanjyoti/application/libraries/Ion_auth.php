...
class Ion_auth
{
	...
	public function get_user_group($value='')
	{
		...
		if(!isset($chk))
		{
			...
			if ($result)  
			{	
				$userPermissions = $this->db->query("call sp_group_permissions(".$result->id.", ".$id.");")->next_result();
				$appParams = (array)fetch_lookup_values('APP_PARAM');

				$group_id = array('username' => $result->first_name.' '.$result->last_name, 'group_id' => $result->id,'group_name' => $result->name, 'permissions' => $userPermissions, 'app_params' => $appParams);
	...
	/*
	* added by Suhrid Sarkar || suhrid.developer@gmail.com on 25-10-21
	* check privilage access for group
	*/
	public function hasPrivilegeOld($perm='', $perm_sub='', $perm_comp='')
	{
		...
		//if permission have 
		if($perm!=''){
			//check if permission have
			$check_perm = $this->ion_auth_model->check_perm($group_id, $perm);
			if($check_perm){
				$flag = TRUE;
				$perm_id = $check_perm;
			}else{
		...
		//if sub permission have 
		if($perm_sub!=''){
			//check if permission have
			$check_sub_perm = $this->ion_auth_model->check_sub_perm($group_id, $perm_id, $perm_sub);
			if($check_sub_perm){
				$flag = TRUE;
				$sub_perm_id = $check_sub_perm;
			}else{
				$flag = FALSE;
			}
		}
		...
		if($perm_comp!=''){
			//check if permission have
			$check_comp_perm = $this->ion_auth_model->check_comp_perm($group_id, $sub_perm_id, $perm_comp);
			if($check_comp_perm){
				$flag = TRUE;
			}else{
				$flag = FALSE;
	...
	public function hasPrivilege($perm='', $perm_sub='', $perm_comp='')
	{
		//get user permission
		...
		foreach($permissions as $permission){
			if($perm!='' && strtolower($permission->perm_name)==strtolower($perm)){
				$permArray['perm'] = $permission->perm_name;
			}
			
			if($perm_sub!='' && strtolower($permission->perm_name)==strtolower($perm) && strtolower($permission->perm_sub_name)==strtolower($perm_sub)){
				$permArray['sub_perm'] = $permission->perm_sub_name;
			}
			
			if($perm_comp!='' && strtolower($permission->perm_name)==strtolower($perm) && strtolower($permission->perm_sub_name)==strtolower($perm_sub) && strtolower($permission->perm_comp_name)==strtolower($perm_comp)){
				$permArray['comp_perm'] = $permission->perm_comp_name;
			}
		}

		//if arg count and match array same then allow to permit
		if(func_num_args()==count($permArray)){
			return TRUE;
		}else{
			return FALSE;
		}
