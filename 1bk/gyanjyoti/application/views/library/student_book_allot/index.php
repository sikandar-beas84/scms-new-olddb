	    ...
	    <div class="main-content-inner">
	        ...
	        <div class="">
	            ...
	            <div class="col-sm-12">
	                ...
	                <!-- <div class="row">
							...
							<div class="col-sm-4">
								<div class="row">
									<div class="form-group">
										<label class="col-sm-4 control-label no-padding-right" for="class"> Select Class </label>
										<div class="col-sm-8">
											<select class="form-control" name="class" id="class" required <?php if(isset($class_id) && $class_id != ''){ echo 'disabled'; }?>>
											  <option value="">-- Select --</option>
                                              <option value="">Select Class</option>
                                            <?php foreach($class as $key=>$v): ?>
                                            <option value="<?=$key;?>"><?=$v;?></option>
							...
							<div class="col-sm-4">
								<div class="row">
									<div class="form-group">
										<label class="col-sm-4 control-label no-padding-right" for="form-field-1"> Student ID </label>
										<div class="col-sm-8">
											<input type="text" class="form-control student-mask-id" name="student_id" id="student_id" required value="<?= ($this->session->userdata('student_logged_in') == true ? $this->session->userdata('code'):'') ?>" <?php if(isset($class_id) && $class_id != ''){ echo 'disabled'; }?>>
											<?php if($this->session->userdata('student_logged_in') == true){ echo '<input type="hidden" name="student_id" value="'. $this->session->userdata('code') .'"/>'; }?>
										</div>
									</div>
								</div>
							...
							<div class="col-sm-4">
							<button class="btn btn-sm btn-primary" type="button" id="submit" onclick="getStudent()">Go</button>
							</div>
							</form>
						</div> -->
	            </div>
	            ...
	            <form class="form-horizontal" role="form" action="javascript:;" id="issueBook">
	                <input type="hidden" name="student_id_hidden" id="student_id_hidden" required>
	                <?php if($this->session->userdata('student_logged_in') != true){ ?>
	                            ...
	                            <tr>
	                                ...
	                                <td>
	                                    <input type="text" name="issue_date[]" id="issue_date" autocomplete="off" required
	                                        class="form-control datepicker1" placeholder="dd/mm/YYYY"
	                                        value="<?= date("d-m-Y"); ?>">
	                                </td>
	                                <td>
	                                    <input type="text" name="valid_date[]" id="valid_date" autocomplete="off" required
	                                        class="form-control datepicker1" placeholder="dd/mm/YYYY"
	                                        value="<?= date("d-m-Y", strtotime($date." +". $library_book_allotment_duration ." day")); ?>">
	                                </td>
	                                <td>
	    ...
	    <div class="modal-dialog" role="document">
	        <div class="modal-content">
	            <div class="modal-header">
	                <h5 class="modal-title" id="exampleModalLabel">Student stationary item invoice</h5>
...
function getStudent() {
    $.ajax({
        method: "POST",
        url: '<?php echo base_url('library/Student_book_allot/getStudent') ?>',
        ...
        if (resp.status == 'Success') {
            ...
            $("#student_id_hidden").val(resp.student_id);
            getIssueBook();
            console.log('1');
            $('html, body').animate({
...
function getIssueBook() {
    $.ajax({
        method: "POST",
        url: '<?php echo base_url('library/Student_book_allot/getIssueBook') ?>',
        data: {
...
$('#fees_items').click(function() {
    ...
    if (amt > 0 && billNo) {
        ...
        if (r == true) {
            ...
            $.ajax({
                url: '<?= base_url() ?>library/student_book_allot/update_issued_items',
                method: 'post',
                data: {
                    selId: chkArray,
                    value: dataString
                ...
                success: function(data) {
                    if (data) {
                        $('.pay_msg').html(
                            '<div class="alert alert-block alert-success"><button data-dismiss="alert" class="close" type="button"><i class="ace-icon fa fa-times"></i></button><p><strong><i class="ace-icon fa fa-check"></i>Success</strong>Fine Collected.</p></div>'
                            );
                        /* setTimeout(function(){										
                            location.reload();			
                        }, 1500); */
                        ...
                        $('.pay_msg').html(
                            '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong><i class="ace-icon fa fa-times"></i> Failed! </strong>Try after sometime<br></div>'
                            );
                    }
                }
            })
        ...
        if (x < maxField) {
            ...
            $(".book").autocomplete({
                source: function(request, response) {
                    jQuery.get(
                        "<?php echo base_url('library/Student_book_allot/getLibrary') ?>", {
                            query: request.term
                        },
                        function(data) {
...
jQuery(function($) {
    ...
    function issueBook() {
        ...
        $.ajax({
            method: "POST",
            url: '<?php echo base_url('library/Student_book_allot/issueBook') ?>',
...
$(function() {
    $(".book").autocomplete({
        source: function(request, response) {
            $.ajax({
                url: "<?php echo base_url('library/Student_book_allot/getLibrary') ?>",
                dataType: "json",
                data: {
                    query: request.term
                ...
                success: function(data) {
                    ...
                    // Ensure the response is an array and contains the proper label and value
                    if (Array.isArray(data)) {
                        response(data); // Pass data to the autocomplete
                    } else {
        ...
        minLength: 1, // Minimum length of the term to trigger the autocomplete
...
function return_item(lib_stock_id, student_id, id) {
    ...
    if (r == true) {
        $.ajax({
            url: "<?php echo base_url('library/student_book_allot/returnItem') ?>",
            method: "post",
            data: {
                id: id,
                lib_stock_id: lib_stock_id,
...
function lost_item(lib_stock_id, fine, id) {
    $.ajax({
        url: "<?=base_url()?>library/student_book_allot/ajax_lost_book_fine",
        method: "post",
        data: {
            lib_stock_id: lib_stock_id,
            fine: fine
...
$('#sub_cpay').click(function() {
    ...
    if (r == true) {
        ...
        var id = $('.id').val();
        $.ajax({
            url: '<?= base_url() ?>library/student_book_allot/add_lost_book_payment',
            method: 'post',
            data: {
                value: dataString,
                id: id
            ...
            success: function(data) {
                if (data) {
                    $('.fine_pay_msg').html(
                        '<div class="alert alert-block alert-success"><button data-dismiss="alert" class="close" type="button"><i class="ace-icon fa fa-times"></i></button><p><strong><i class="ace-icon fa fa-check"></i>Success</strong>Your Transaction is Successfull . </p></div>'
                        );
                    setTimeout(function() {
                        location.reload();
                    }, 4000);
                    ...
                    $('.fine_pay_msg').html(
                        '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button><strong><i class="ace-icon fa fa-times"></i> Failed! </strong>Their is an error for this transaction<br></div>'
                        );
                }
            }
        })
