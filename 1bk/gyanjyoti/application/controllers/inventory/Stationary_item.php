...
class Stationary_item extends CI_Controller {
	...
	public function list(){
        ...
        # customize filter
        $order_by = array('stationary_item.id ' => 'desc');
        $where_in = array();
		$where = array('stationary_item.is_delete' => 'N');

        $join = array(
            
            array(
				'ontable'	=> 'store',
				'onParams'	=> 'store.id = stationary_item.store',
				'type'		=> 'left'
			),
        );
 
        $queryAttachments = array(
            'select' => 'stationary_item.*, store.name as store_name', // Fetch store name along with stationary item details
            'where' => $where,
            'where_in' => $where_in,
            'join' => $join,
            'group_by' => ''
        ...
        $dttbl_model = new Datatable_model('stationary_item', array(), array(), $order_by, $queryAttachments);
		...
		. '</div>';
            ...
            $data[] = array(
                ...
                $fieldData->item_name,
                $fieldData->unit,
                $fieldData->openning_stock,
                $fieldData->re_oreder_level,
				$action
        ...
        $output = array(
            ...
            "data" => $data,
            "status" => 'success',
			// "csrf" => update_csrf_stationary_item()
        );
        ...
        public function action(){
            $data['table_name'] = $table_name = 'stationary_item'; // Replace with your table name
            ...
            if(post('action') == 0){ 
                ...
                $res = $this->Generalmodel->getData('stationary_item',post('id'),'id','','','update',$fromData);
                if($res):
                    $result = array('html' => '', 'message'=>'stationary_item Delete Successfully.', 'status' => 'success');
                else:
                    $result = array('html' => $this->db->last_query(), 'message'=>'Something went to wrong.', 'status' => 'error');
                endif;
            }elseif(post('action') == 2){
                $data['data'] = $this->Generalmodel->getData('stationary_item',post('id'),'id','','','get','');
