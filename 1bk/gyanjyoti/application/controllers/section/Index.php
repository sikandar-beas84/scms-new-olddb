...
class Index extends CI_Controller {
	...
	public function index(){
        ...
        $head['title'] = $data['page_title'] = 'section';
	...
	public function list(){
        ...
        # customize filter
        $order_by = array('section.id ' => 'desc');
        $where_in = array();
		$where = array('section.is_delete' => 'N');

        $join = array(
            array(
				'ontable'	=> 'session',
				'onParams'	=> 'session.id = section.session_year',
				'type'		=> 'left'
			),
            array(
				'ontable'	=> 'class',
				'onParams'	=> 'class.id = section.class',
				'type'		=> 'left'
			),
            array(
				'ontable'	=> 'staff',
				'onParams'	=> 'staff.id = section.teacher',
				'type'		=> 'left'
			),
        );
 
        $queryAttachments = array(
            'select' => 'section.*,session.start_year, session.end_year, class.class_name, staff.first_name, staff.last_name',
            'where' => $where,
            'where_in' => $where_in,
            'join' => $join,
            'group_by' => ''
        ...
        $dttbl_model = new Datatable_model('section', array(), array(), $order_by, $queryAttachments);
        ...
        public function action(){
            $data['table_name'] = $table_name = 'section'; // Replace with your table name
            ...
            if(post('action') == 0){ 
                ...
                $res = $this->Generalmodel->getData('section',post('id'),'id','','','update',$fromData);
                if($res):
                    $result = array('html' => '', 'message'=>'Ssection Delete successfully.', 'status' => 'success');
                else:
                    $result = array('html' => '', 'message'=>'Something went to wrong.', 'status' => 'error');
                endif;
            }elseif(post('action') == 2){
                $data['data'] = $this->Generalmodel->getData('section',post('id'),'id','','','get','');
        ...
        function save() {
            $table = 'section'; // Specify your table name here
	...
	function deleteImage(){
		...
		$action = $this->Generalmodel->getData('section',post('id'), 'id', '', '', 'update', $data);
		if($action){
