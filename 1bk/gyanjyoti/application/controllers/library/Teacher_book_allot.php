...
class Teacher_book_allot extends CI_Controller {
	...
	public function index(){
        ...
        $head['title'] = $data['page_title'] = 'Teacher Book Assign';
	...
	public function getTeacher(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$teacher = $this->Library_model->get_teacher_details($this->input->post('id'));	
			if(!empty($teacher)){
				
				$html = '<div class="profile-user-info profile-user-info-striped">';
				$html .= '<div class="profile-info-row">
												<div class="profile-info-name"> Teacher Name </div>

												<div class="profile-info-value">
													<span class="editable editable-click" id="username">'.$teacher->first_name . ' ' . $teacher->last_name . '</span>
												</div>
											</div>';
				// $html .= '<div class="profile-info-row">
				...
				// 								<div class="profile-info-value">
				// 									<span class="editable editable-click" id="username">'. $teacher->code .'</span>
				// 								</div>
				// 							</div>';
				$html .= '<div class="profile-info-row">
												<div class="profile-info-name"> Teacher Email </div>

												<div class="profile-info-value">
													<span class="editable editable-click" id="username">'.$teacher->email.'</span>
												</div>
											</div>';
				// $html .= '<div class="profile-info-row">
				// 								<div class="profile-info-name"> Subject Tought </div>

				// 								<div class="profile-info-value">
				// 									<span class="editable editable-click" id="username">'. $teacher->subject_tought .'</span>
				// 								</div>
				// 							</div>';
				// $html .= '<div class="profile-info-row">
				// 								<div class="profile-info-name"> Whatsapp No. </div>

				// 								<div class="profile-info-value">
				// 									<span class="editable editable-click" id="username">'.$teacher->phone_no_other.'</span>
				// 								</div>
				// 							</div>';
				$html .= '<div class="profile-info-row">
												<div class="profile-info-name"> Phone No. </div>
												<div class="profile-info-value">
													<span class="editable editable-click" id="username">'.$teacher->phone.'</span>
												</div>
											</div>';
				$html .= "</div>";
				$data['data'] = $html;
				$data['id'] = $teacher->code;
				$data['status'] = 'Success';
				echo json_encode($data); 
				die;	
			} else {
				$data['data'] = "<p>No teacher found!</p>";
				$data['status'] = 'Failure';
				echo json_encode($data); 
				die;
			}			
	...
	public function getTeacherIssueBook(){
		
		$issue_ds = $this->Library_model->get_book_for_teacher($this->input->post('teacher_id'));	
			...
			foreach($issue_ds as $data1){
				...
				if($data1->status=='issued'){
					...
					if($fine>0 && $data1->invoice_status==1){
						$html .= '<span class="btn btn-sm btn-warning h3"  href="javascript:void(0);" onclick="return_item(\''.$data1->lib_stock_id.'\',\''.$data1->teacher_id.'\',\''.$data1->id.'\');">Return </span>';
					}else if($fine==0){
						$html .= '<span class="btn btn-sm btn-warning  h3"  href="javascript:void(0);" onclick="return_item(\''.$data1->lib_stock_id.'\',\''.$data1->teacher_id.'\',\''.$data1->id.'\');">Return </span>';
					}
    ...
    public function issueBookTeacher()	{	
		...
		if ($this->form_validation->run() == FALSE) {
			...
			for($i=0;$i<count($_POST['book']);$i++){
				...
				$issue_data['teacher_id'] = $this->input->post('teacher_id_hidden');
				...
				$result = $this->Library_model->save_issue_teacher($issue_data);
	...
	public function returnItem(){
		...
		$issue_ds = $this->Library_model->get_return_details_teacher($this->input->post('id'));	
		// pr($issue_ds);
		$library_books_fine_per_day = $this->Generalmodel->get_configuration_by_key('library_books_fine_per_day');
		$date_now = date("Y-m-d");				
		if ($date_now > $issue_ds->valid_date) {
			...
			$student_ds = $this->Library_model->get_staff_details('',$student_id);
		...
		$this->db->update('lib_book_allot_teacher'); 
		...
		if(!empty($issue_ds)){
				...
				$html .='<script>
					...
					$(".selected").on("click", function(e){
						...
						if(selectedArr.length > 0){
							$.ajax({
								url:"'. base_url().'library/teacher_book_allot/fine_amt_for_sel_mnth",									
								method: "post",					
								data: {ids : selectedArr},
								success: function(data){
									var data =  $.parseJSON( data );
    ...
    public function add_lost_book_payment(){
		...
		$result = $this->Library_model->update_issued_items_teacher($id,$updata);	
