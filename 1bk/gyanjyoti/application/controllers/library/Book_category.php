...
class Book_category extends CI_Controller {
	...
	public function list(){
        ...
        # customize filter
        $order_by = array('book_category.id ' => 'desc');
        $where_in = array();
		$where = array('book_category.is_delete' => 'N');
        ...
        $dttbl_model = new Datatable_model('book_category', array(), array(), $order_by, $queryAttachments);
        ...
        $output = array(
            ...
            "data" => $data,
            "status" => 'success',
			// "csrf" => update_csrf_book_category()
        );
        ...
        public function action(){
            $data['table_name'] = $table_name = 'book_category'; // Replace with your table name
            ...
            if(post('action') == 0){ 
                ...
                $res = $this->Generalmodel->getData('book_category',post('id'),'id','','','update',$fromData);
                if($res):
                    $result = array('html' => '', 'message'=>'book_category Delete Successfully.', 'status' => 'success');
                else:
                    $result = array('html' => $this->db->last_query(), 'message'=>'Something went to wrong.', 'status' => 'error');
                endif;
            }elseif(post('action') == 2){
                $data['data'] = $this->Generalmodel->getData('book_category',post('id'),'id','','','get','');
