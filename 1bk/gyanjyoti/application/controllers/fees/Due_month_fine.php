...
class Due_month_fine extends CI_Controller {
	...
	public function list(){
        ...
        # customize filter
        $order_by = array('month_wise_due_date.id ' => 'desc');
        $where_in = array();
		$where = array('month_wise_due_date.is_delete' => 'N');
		$where = array('month_wise_due_date.session_year' => $this->session->userdata('session'));

        $join = array(
            array(
				'ontable'	=> 'session',
				'onParams'	=> 'session.id = month_wise_due_date.session_year',
				'type'		=> 'left'
			)
        ...
        $queryAttachments = array(
            'select' => 'month_wise_due_date.*,session.start_year, session.end_year',
            'where' => $where,
            'where_in' => $where_in,
            'join' => $join,
            'group_by' => ''
        ...
        $dttbl_model = new Datatable_model('month_wise_due_date', array(), array(), $order_by, $queryAttachments);
		...
		// prx($fieldData->gallery);
			...
			$image = '<div class="d-flex align-items-center">
                  <img
                      src="' . base_url() . 'assets/uploads/month_wise_due_date/'.$image.'"
                      alt=""
		...
		. '</div>';
            ...
            if($fieldData->is_delete == 'N'){
                    ...
                    $month = isset($months[$fieldData->month]) ? $months[$fieldData->month] : 'Invalid Month';
                $data[] = array(
                    $key + 1,
                    $fieldData->start_year . ' - ' . $fieldData->end_year,
                    $month,
                    $fieldData->due_date,
                    $fieldData->fine_amount,
                    $action
        ...
        $output = array(
            ...
            "data" => $data,
            "status" => 'success',
			// "csrf" => update_csrf_month_wise_due_date()
        );
	...
	// For Delete, Update and View From DataTable
        ...
        public function action(){
            $data['table_name'] = $table_name = 'month_wise_due_date'; // Replace with your table name
            ...
            if(post('action') == 0){ 
                ...
                $res = $this->Generalmodel->getData('month_wise_due_date',post('id'),'id','','','update',$fromData);
                if($res):
                    $result = array('html' => '', 'message'=>'month_wise_due_date Delete Successfully.', 'status' => 'success');
                else:
                    $result = array('html' => '', 'message'=>'Something went to wrong.', 'status' => 'error');
                endif;
            }elseif(post('action') == 2){
                $data['data'] = $this->Generalmodel->getData('month_wise_due_date',post('id'),'id','','','get','');
            ...
            // $obj = (object) array_merge((array) $result,update_csrf_month_wise_due_date());
            echo json_encode($obj);
        }

        function save() {
            $table = 'month_wise_due_date'; // Specify your table name here
...
public function file_check($str) {
	...
	function deleteImage(){
		...
		$action = $this->Generalmodel->getData('month_wise_due_date',post('id'), 'id', '', '', 'update', $data);
		if($action){
