...
class Content extends CI_Controller {
	...
	public function list(){
        ...
        $where_in = array();
		$where = array('content.is_delete' => 'N');
        ...
        $dttbl_model = new Datatable_model('content', array(), array(), $order_by, $queryAttachments);
		...
		// prx($fieldData->gallery);
			...
			$image = '<div class="d-flex align-items-center">
                  <img
                      src="' . base_url() . 'assets/uploads/content/'.$image.'"
                      alt=""
...
// 		. '<a href="javascript:" class="btn btn-outline-danger btn-xs" onclick="action('.$fieldData->id.',0)"><i class="fa fa-trash" aria-hidden="true"></i></a>'
        ...
        public function action(){
            $data['table_name'] = $table_name = 'content'; // Replace with your table name
            ...
            // prx($data['select']);
            $data['textEditor'] = ['body'];
            $data['unsetTextEditor'] = ['body'];
            $data['fieldType'] = [];
            $data['size'] = ['body' => '12', 'status' => 12, 'title'=> 12];
            $data['dynamic_fields'] = [];
            $data['hidden'] = 
            [
            'id',
            ...
            $data['file'] = array(
                'file' => array(
                    'fieldType' => 'sigle',
                    'accept' => '',
                    'path' => base_url().'assets/uploads/content/',
            ...
            if(post('action') == 0){ 
                ...
                $res = $this->Generalmodel->getData('content',post('id'),'id','','','update',$fromData);
                if($res):
                    $result = array('html' => '', 'message'=>'content Delete successfully.', 'status' => 'success');
                else:
                    $result = array('html' => '', 'message'=>'Something went to wrong.', 'status' => 'error');
                endif;
            }elseif(post('action') == 2){
                $data['data'] = $this->Generalmodel->getData('content',post('id'),'id','','','get','');
        ...
        function save() {
            $table = 'content'; // Specify your table name here
...
public function file_check($str) {
	...
	function deleteImage(){
		...
		$action = $this->Generalmodel->getData('content',post('id'), 'id', '', '', 'update', $data);
		if($action){
