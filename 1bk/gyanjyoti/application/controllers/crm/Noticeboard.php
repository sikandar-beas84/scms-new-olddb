...
class Noticeboard extends CI_Controller {
	...
	public function list(){
        ...
        # customize filter
        $order_by = array('notice_board.id ' => 'desc');
        $where_in = array();
		$where = array('notice_board.is_delete' => 'N');
        ...
        $dttbl_model = new Datatable_model('notice_board', array(), array(), $order_by, $queryAttachments);
		...
		. '</div>';
            ...
            $data[] = array(
                $key + 1,
                $fieldData->notice_title,
                $fieldData->notice_date ,
	...
	// For Delete, Update and View From DataTable
        ...
        public function action(){
            $data['table_name'] = $table_name = 'notice_board'; // Replace with your table name
            ...
            if(post('action') == 0){ 
                ...
                $res = $this->Generalmodel->getData('notice_board',post('id'),'id','','','update',$fromData);
                if($res):
                ...
                $data['data'] = $this->Generalmodel->getData('notice_board',post('id'),'id','','','get','');
        ...
        function save() {
            $data['table_name'] = $table = 'notice_board'; // Replace with your table name
...
public function file_check($str) {
	...
	function deleteImage(){
		...
		$action = $this->Generalmodel->getData('notice_board',post('id'), 'id', '', '', 'update', $data);
		if($action){
