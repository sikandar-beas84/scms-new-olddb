...
class LabItem extends CI_Controller {
	...
	public function list(){
        ...
        # customize filter
        $order_by = array('labitem_master.id ' => 'desc');
        $where_in = array();
		$where = array('labitem_master.is_delete' => 'N');

        $join = array();
 
        $queryAttachments = array(
            'select' => 'labitem_master.*',
            'where' => $where,
            'where_in' => $where_in,
            'join' => $join,
            'group_by' => ''
        ...
        $dttbl_model = new Datatable_model('labitem_master', array(), array(), $order_by, $queryAttachments);
        ...
        $output = array(
            ...
            "data" => $data,
            "status" => 'success',
			// "csrf" => update_csrf_labitem_master()
        );
        ...
        public function action(){
            $data['table_name'] = $table_name = 'labitem_master'; // Replace with your table name
            ...
            if(post('action') == 0){ 
                ...
                $res = $this->Generalmodel->getData('labitem_master',post('id'),'id','','','update',$fromData);
                if($res):
                    $result = array('html' => '', 'message'=>'labitem_master Delete Successfully.', 'status' => 'success');
                else:
                    $result = array('html' => $this->db->last_query(), 'message'=>'Something went to wrong.', 'status' => 'error');
                endif;
            }elseif(post('action') == 2){
                $data['data'] = $this->Generalmodel->getData('labitem_master',post('id'),'id','','','get','');
