...
class Index extends CI_Controller {
	...
	public function index(){
        ...
        $head['title'] = $data['page_title'] = 'class';
        $data['class']  = $this->Common_model->getAllData('class', '', '', ['status'=> 'Y']);
	...
	public function list(){
        ...
        # customize filter
        $order_by = array('class.id ' => 'desc');
        $where_in = array();
		$where = array('class.is_delete' => 'N');
        ...
        $dttbl_model = new Datatable_model('class', array(), array(), $order_by, $queryAttachments);
        ...
        public function action(){
            $data['table_name'] = $table_name = 'class'; // Replace with your table name
            ...
            $data['fieldType'] = [];
            $data['size'] = ['class_name'=> '12','status' => '12'];
            ...
            if(post('action') == 0){ 
                ...
                $res = $this->Generalmodel->getData('class',post('id'),'id','','','update',$fromData);
                if($res):
                    $result = array('html' => '', 'message'=>'Class Delete successfully.', 'status' => 'success');
                else:
                    $result = array('html' => $this->db->last_query(), 'message'=>'Something went to wrong.', 'status' => 'error');
                endif;
            }elseif(post('action') == 2){
                $data['data'] = $this->Generalmodel->getData('class',post('id'),'id','','','get','');
        ...
        function save() {
            $table = 'class'; // Specify your table name here
